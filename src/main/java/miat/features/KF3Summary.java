...
public class KF3Summary {
    public static void idLookup(String idOrName, MessageCreateEvent mc) {
        ...
        //Populate the maps with data from the chara data list
        for (int i = 0; i < charaData.length(); i++) {
            ...
            String name = jsonObject.getString("nameEn");
            nameMap.put(name, jsonObject);
        }
        //determine if we need to read from name or id map
        try {
        ...
        //need to have a catch block in case some bullshit error happens
        try {
            if (isID) {
                int id = Integer.parseInt(idOrName);
                JSONObject characterInfo = idMap.get(id);
                String nameJP = characterInfo.get("name").toString();
                String nameEN = characterInfo.get("nameEn").toString();
                String sciName = characterInfo.get("animalScientificName").toString();
                e.setDescription("ID: " + id + "\nName JP: " + nameJP + "\nName EN: " + nameEN + "\nScientific Name: " + sciName);
                e.setColor(Color.green);
                new Reply().noPing(mc, e);
                return;
            } else {
                for (String individual : nameMap.keySet()) {
                    String gameNameSanitized = individual.toLowerCase().replace(" ","").replace("-","").replace("é","e").replace("'", "");
                    String userNameSanitized = idOrName.toLowerCase().replace(" ","").replace("-","").replace("é","e").replace("'", "");
                    if (userNameSanitized.equals(gameNameSanitized)) {
                        JSONObject characterInfo = nameMap.get(individual);
                        int id = characterInfo.getInt("id");
                        String nameJP = characterInfo.get("name").toString();
                        String nameEN = characterInfo.get("nameEn").toString();
                        String sciName = characterInfo.get("animalScientificName").toString();
                        e.setDescription("ID: " + id + "\nName JP: " + nameJP + "\nName EN: " + nameEN + "\nScientific Name: " + sciName);
            ...
            e.setDescription("Invalid character id or name supplied.");
            new Reply().noPing(mc, e);
        } catch (Exception ex) {
            e.setDescription("this could be one of a few things, probably the id or name you put in doesnt exist, the " +
                    "entry for this friend is incomplete, or another json parsing error has occurred.");
            e.setColor(Color.red);
            new Reply().noPing(mc, e);
        }
    }
    public static void friendStory(String friendId, MessageCreateEvent mc) {
        Map<Integer, JSONObject> idMap = new HashMap<>();
        Map<String, JSONObject> nameMap = new HashMap<>();
        JSONArray charaData = new JSONArray(Reader.readFull("CHARA_DATA.json"));
        ...
        for (int iter = 0; iter < 1; iter++) {
            ...
            mc.getMessage().getChannel().sendMessage("Summarized character story.", inputStream, filename);
        }
    }
